...
const TvShowReviewsPage = () => {
  ...
  const fetchReviews = async (page = 1) => {
    try {
      ...
      const res = await api.get(`/admin/tvshowreviews?${queryParams}`)
      ...
      setError("Failed to load TV show reviews")
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    try {
      await api.delete(`/tvshowreviews/${reviewId}`)
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Manage TV Show Reviews</h1>
      ...
      <div className="bg-purple-800 rounded-lg overflow-hidden shadow-lg mb-6">
        {loading ? (
              ...
              <tbody className="divide-y divide-purple-700/30">
                {reviews.map((review) => (
                  <tr key={review._id} className="hover:bg-purple-700/30">
                    ...
                    <td className="p-4">
                      <Link to={`/tv-shows/${review.tvShowId._id}`} className="hover:text-yellow-400">
                        {review.tvShowId.title}
                      </Link>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <div className="flex space-x-2">
                        <Link
                          to={`/tv-shows/${review.tvShowId._id}`}
          ...
          <div className="p-8 text-center">
            <p className="text-gray-300">No TV show reviews found matching your criteria.</p>
          </div>
        )}
      </div>
